...
- **Über die Weboberfläche (empfohlen):** Seite **„Werksreset"** → Option **„gespeicherte WLAN-Netzwerke zurücksetzen"**. Löscht nur die WLAN-Zugangsdaten, alle Zifferblätter/Zeigersätze/Presets bleiben erhalten. Die Uhr geht danach wieder in den WPS-/Accesspoint-Modus (siehe 1.1).
...
- **Via the web interface (recommended):** Go to the **"Factory Reset"** page → option **"reset saved WiFi networks"**. This only erases the WiFi credentials; all clock faces/hand sets/presets are kept. The clock then goes back into WPS/access point mode (see 1.1).
